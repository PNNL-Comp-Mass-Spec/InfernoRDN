  ...
  partial class frmDAnTE
  {
      ...
      try {
        ...
        try {
          ...
          if (rConnector.GetTableFromRProtInfoMatrix("ProtInfo")) {
            ...
            mDTProts.TableName = "ProtInfo";
            if (mDTProts.Columns.CanRemove(mDTProts.Columns[0])) {//remove the line number column which comes from R row names.
    ...
    private DataTable GetProtInfoMatrix()
    {
      ...
      if (rConnector.GetTableFromRProtInfoMatrix("ProtInfo")) {
        ...
        if (mDTProtInfo.Columns.CanRemove(mDTProtInfo.Columns[0])) {//remove the line number column which comes from R row names.
    ...
    private void UpdateFactorInfoArray()
    {
      ...
      try {
        ...
        for (int numF = 0; numF < factors.Length; numF++) {
          ...
          if ((int)nfVals[0] > 1) { //more than one factor value
